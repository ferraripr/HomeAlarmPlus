...
namespace AlarmByZones
{
    ...
    public class EventLogger
    {
        ...
        private void RecurseFolders(DirectoryInfo directory)
        {
            if (directory.Exists)
            {
                ...
                foreach (FileInfo file in directory.GetFiles())
                {
                    ...
                    string htmlCheckbox = " <input type=\"checkbox\" name=\"filegroup\" value=\"" + file.FullName + "\"  />";
                    ...
                    string filename = file.FullName == Alarm.User_Definitions.Constants.ALARM_CONFIG_FILE_PATH ||
                        file.FullName == Alarm.User_Definitions.Constants.HTML_RESOURCE_HEADER_STYLE ||
                        file.FullName == Alarm.User_Definitions.Constants.HTML_RESOURCE_TABLE_STYLE ?
                        file.FullName :
        ...
        {
            string path = type.ToUpper() == "EXCEPTION" ? "\\SD\\Exception" : "\\SD\\Logs";
            CheckDirectory(path);
            try
            {
                using (StreamWriter sw = new StreamWriter(path + "\\" + sourceFileName))
                {
            ...
            if (IsSDCardAvailable())
            {
                ...
                foreach (VolumeInfo vi in volumes)
                {
                    ...
                    Info = "<br/><b>SD Card Status</b>" +
                        ...
                        "<li>Free Space: " + FreeSpace.ToString() + "MB</li>";
                }
            }
            else
            {
